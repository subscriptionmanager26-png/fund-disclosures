#!/usr/bin/env python3
"""Azure Blob Storage Exporter for Fund Disclosures.

Uploads raw spreadsheets and contract-shaped Parquet files to Azure Blob Storage.
"""
...
def export_azure(container_name: str, period: str, cadence: str = "monthly", amc_filter: Optional[str] = None):
    ...
    print(f"\n=== [Azure Blob Export] Period={period}, Container={container_name} ===")
        ...
        for date_dir in [d for d in raw_base.iterdir() if d.is_dir() and period in d.name]:
            ...
            for amc_dir in date_dir.iterdir():
                ...
                for file_path in amc_dir.iterdir():
                    ...
                    raw_count += 1
                    print(f"  [Azure Raw] Uploaded: {blob_path}")
        ...
        for date_dir in [d for d in parsed_base.iterdir() if d.is_dir() and period in d.name]:
            for amc_dir in date_dir.iterdir():
                ...
                for scheme_dir in amc_dir.iterdir():
                    ...
                    print(f"  [Azure Parquet] Uploaded: {blob_path} ({len(df)} rows)")

    print(f"\n[Azure Result] Raw Files: {raw_count}, Parquet Schemes: {norm_count}, Total Holdings: {total_holdings}")


def main() -> int:
    ap = argparse.ArgumentParser(description="Export to Azure Blob Storage")
    ap.add_argument("--container", required=True, help="Target Azure container name")
    ...
    export_azure(args.container, args.period, args.cadence, args.amc)
    return 0
